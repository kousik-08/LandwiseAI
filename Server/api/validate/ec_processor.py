...
class ECProcessor:
    ...
    # v11: each transaction now carries `plot_no` (மனை எண் / Plot No, read from
    #      its own schedule block) so the Ownership Audit can group a survey's
    #      transactions BY PLOT and show each plot's own mother→current chain.
    #      The RAW_PROMPT already emits "Plot No:", so this RE-PARSES the cached
    #      OCR — no Gemini re-OCR needed (OCR_VERSION unchanged).
    _PARSE_VERSION = 11
    ...
    def parse_to_json(self, raw_text: str) -> List[Dict[str, Any]]:
        ...
        for _tok in token_re.finditer(raw_text):
            ...
            base_tx: Dict[str, Any] = {
                ...
                # மனை எண் / Plot No from this transaction's own schedule block.
                # Lets the Ownership Audit group a survey's deeds BY PLOT.
                "plot_no": self._extract_field(block, "Plot No"),
