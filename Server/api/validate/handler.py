...
def _run_async_blocking(coro):
    """Run a coroutine to completion from sync code, regardless of whether an
    event loop is already running on the calling thread. workflow_generator is a
    SYNC generator that may be driven either on a worker thread or inside the
    async route, so we run the coroutine on a dedicated thread with its own loop
    to avoid 'event loop is already running' errors."""
    import asyncio
    import threading
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            report_data = _run_async_blocking(handle_generate_report(processing_id))
            if isinstance(report_data, dict) and report_data.get("status") == "success":
                ...
                # The summary is written to legal/opinion_summary.json on disk here.
                # Its AnalysisResult DB row (which requires parcel_id, not available
                # in this generator) is persisted later in
                # persist_forensic_results_to_db, which reads that JSON artifact.
...
async def persist_forensic_results_to_db(parcel_id: str, request_id: str, output_dir: str):
    ...
    try:
        ...
        # 0.5 Opinion summary (verdict + issues + pros). The analyze stage wrote
        # legal/opinion_summary.json; mirror it to an AnalysisResult row WITH
        # parcel_id so the Overview tab's DB fallback survives scratch cleanup.
        try:
            opinion_summary_path = os.path.join(output_dir, "legal", "opinion_summary.json")
            if os.path.exists(opinion_summary_path):
                with open(opinion_summary_path, "r", encoding="utf-8") as f:
                    _osummary = json.load(f)
                existing_os = db.query(AnalysisResult).filter(
                    AnalysisResult.request_id == request_id,
                    AnalysisResult.result_type == 'opinion_summary',
                ).first()
                if existing_os:
                    existing_os.data = _osummary
                    existing_os.parcel_id = parcel_id
                else:
            ...
            print(f"[!] Opinion summary persist failed (non-fatal): {_ose}")
            db.rollback()
        ...
        if cache_entry and cache_entry.get("pipeline_version") == CACHE_PIPELINE_VERSION:
            ...
            if cached_result is not None:
                ...
                if parcel_id and cached_rid:
                    ...
                    try:
                        # Point the parcel at the CACHED run (cached_rid), not the
                        # new processing_id. A cache hit produces no artifacts under
                        # processing_id, so setting last_analysis_request_id to it
                        # makes every parcel-keyed GET (/hierarchy, /opinion-summary,
                        # ...) resolve to an empty request and 404. cached_rid is
                        # where the EC/hierarchy/results/legal artifacts actually live.
                        _register_documents_for_parcel(
                            parcel_id, cached_rid, actual_ec_pdf_path,
