...
# Bump this token whenever the validation PROMPT changes. The cache key below is
# otherwise only (ec_entry + extracted metadata), so a prompt-only change would
# NOT invalidate cached results — every document would keep returning its old
# verdict. Including the version here forces a one-time re-validation after a
# prompt edit. ("extent-no-sum": stop summing EC schedules into a total.)
VALIDATION_PROMPT_VERSION = "2026-06-23-extent-no-sum"
...
def _inputs_hash(ec_entry: Dict, metadata_content: str) -> str:
    ...
    Includes VALIDATION_PROMPT_VERSION so a prompt change also busts the cache.
    """
    payload = (
        VALIDATION_PROMPT_VERSION
        + "\x00"
        + json.dumps(ec_entry or {}, sort_keys=True, ensure_ascii=False)
        + "\x00"
        + (metadata_content or "")
    )
