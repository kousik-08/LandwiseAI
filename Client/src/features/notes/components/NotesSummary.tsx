    ...
    return (
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-amber-50/60 via-white to-white shrink-0">
                ...
                {/* Stat pills — customized for notes (mirrors the Document
                    Analysis header's stat row, but with note-specific metrics
                    instead of MATCHED / REVIEW / TRUST). */}
                {(() => {
                    const allNotes = data.documents.flatMap((d) => d.notes);
                    const resolvedCount = allNotes.filter((n) => n.is_resolved).length;
                    const openCount = allNotes.length - resolvedCount;
                    const pills: { label: string; value: number; cls: string }[] = [
                        { label: "Total Notes", value: data.total_notes, cls: "border-indigo-100 bg-indigo-50 text-indigo-700" },
                        { label: "Documents", value: data.documents_with_notes, cls: "border-slate-200 bg-slate-50 text-slate-700" },
                        { label: "Open", value: openCount, cls: "border-amber-100 bg-amber-50 text-amber-700" },
                        { label: "Resolved", value: resolvedCount, cls: "border-emerald-100 bg-emerald-50 text-emerald-700" },
                    ];
                    return (
                        <div className="flex flex-wrap items-center gap-2 mt-3">
                            {pills.map((p) => (
                                ...
                                >
                                    ...
                                    <span className="text-sm font-extrabold tabular-nums leading-none">{p.value}</span>
                                </div>
                            ))}
                        </div>
                    );
                            ...
                            <div className="flex-1 min-h-0 relative">
                                {previewPdfUrl ? (
                                    ...
                                    <PdfAnnotator
                                        ...
                                        // No popup card — the note's region is marked inline on the
                                        // (now full-size) PDF and its text + Remove live in the left
                                        // list, mirroring the Document Analysis layout.
                                    />
                                ) : (
