...
// Set PDF.js Worker. Bundle the worker that EXACTLY matches the installed
// pdfjs-dist (4.4.168 — the version react-pdf-highlighter@8 requires) via Vite's
// `?url` import, so the worker can never drift from the library version. The
// old hardcoded CDN URL pinned a different version, which surfaced as
// "The API version ... does not match the Viewer version ..." crashes.
...
interface PdfAnnotatorProps {
    ...
    /** When true, focusing a note (via focusHighlightId) also auto-opens its
     *  observation card — parity with hovering the highlight, so clicking a
     *  note in a list surfaces its text + Remove action. The Notes Hub passes
     *  this; Document Analysis / AnalysisDashboard leave it off (unchanged). */
    showTipOnFocus?: boolean;
}
...
const PdfAnnotator: React.FC<PdfAnnotatorProps> = ({
    ...
    // The note's observation card — rendered both on hover (Popup, below) and,
    // when `showTipOnFocus` is set, auto-opened on a list click via the
    // library's showTip(). Shared so both paths render an identical card.
    const renderObservationCard = (h: IHighlight) => (
        <div className="anno-popup-card animate-in fade-in slide-in-from-bottom-1 duration-200">
            ...
            <div className="anno-popup-content">{h.comment.text}</div>
            <Button
                variant="destructive"
                size="sm"
        ...
        const tryFocus = () => {
            ...
            if (target && highlighterRef.current) {
                try {
                    ...
                    // Parity with Document Analysis: when the caller opts in,
                    // auto-open this note's observation card (the same card hover
                    // shows) so a list click surfaces its text + Remove action
                    // without a hover. Real notes only — skip validation-overlay
                    // ids and empty comments. Deferred so the scroll settles
                    // before the tip is positioned (and the re-added scroll
                    // listener doesn't immediately dismiss it).
                    if (showTipOnFocus && target.comment?.text && !String(target.id).startsWith("vd-")) {
                        const inst = highlighterRef.current as any;
                        tipTimer = setTimeout(() => {
                            if (cancelled || !inst?.showTip || !inst?.scaledPositionToViewport) return;
                            try {
                                const vp = inst.scaledPositionToViewport(target.position);
                                inst.showTip({ ...target, position: vp }, renderObservationCard(target));
                            } catch (err) {
                                console.warn("auto-open observation tip failed", err);
                            }
                        }, 300);
                    }
                    return;
                    ...
                    <PdfHighlighter
                        ...
                        highlightTransform={(
                            ...
                            return (
                                <Popup
                                    key={index}
                                    popupContent={renderObservationCard(highlight)}
