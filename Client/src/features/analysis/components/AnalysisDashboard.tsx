...
import { ECHistoricalValues } from "./ECHistoricalValues";
import { ValueComparisonAudit } from "./ValueComparisonAudit";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { MapPin, ExternalLink, Loader2, FileText, Calendar, User, ShieldCheck, AlertCircle, TrendingUp } from "lucide-react";
...
export function ValidationResults({ results, red_flags = [], hierarchyPath, requestId }: ValidationResultsProps) {
  ...
  const fetchValuationData = async () => {
    ...
    try {
      const resp = await fetch(`${API_BASE_URL}/api/v1/analyze-ec`, {
        ...
        // Usually stored as ec.pdf in the inputs folder
        body: new URLSearchParams({
          ec_pdf_path: `inputs/validate/${requestId}/ec.pdf`
        })
      });
      if (resp.ok) {
        const data = await resp.json();
        ...
        toast.error("EC Analysis failed. Ensure 'ec.pdf' exists in request inputs.");
      }
    } catch (e) {
  ...
  const rawPdfUrl = getFileUrl(selectedResult?.file_path);
  const pdfUrl = rawPdfUrl
    ? (selectedPage !== null
      ? `${rawPdfUrl}?t=${refreshKey}#page=${selectedPage === 0 ? 1 : selectedPage}&navpanes=0`
      : `${rawPdfUrl}?t=${refreshKey}#navpanes=0`)
    : null;

  const hierarchyUrl = getFileUrl(hierarchyPath);
            ...
            <ResizablePanelGroup direction="horizontal" className="w-full">
              <ResizablePanel defaultSize={45} minSize={30}>
                <div className="h-full overflow-y-auto p-6 scrollbar-thin">
                  ...
                  {/* Advanced Red Flags Section */}
                  {red_flags.length > 0 && (
                    <div className="mb-6 space-y-3 bg-red-50/50 border border-red-100 p-4 rounded-2xl">
                      <div className="flex items-center gap-2 mb-1">
                        <ShieldCheck className="w-4 h-4 text-red-600" />
                        <span className="text-xs font-black text-red-900 uppercase tracking-tighter">Chain of Title Risk Alerts</span>
                        <Badge variant="outline" className="ml-auto bg-red-100 text-red-700 border-red-200 font-bold">
                          {red_flags.length}
                        </Badge>
                      </div>
                      <div className="space-y-2">
                        {red_flags.map((flag, idx) => (
                          <div key={idx} className="p-3 bg-white border border-red-100 rounded-xl shadow-sm hover:shadow-md transition-all">
                            <div className="flex items-start gap-3">
                              <AlertCircle className={cn(
                                "w-4 h-4 shrink-0 mt-0.5",
                                flag.severity === "CRITICAL" ? "text-red-600 animate-pulse" : "text-orange-500"
                              )} />
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <Badge className={cn(
                                    "text-[8px] h-3.5 px-1 font-bold",
                                    flag.severity === "CRITICAL" ? "bg-red-600" : "bg-orange-500"
                                  )}>
                                    {flag.severity}
                                  </Badge>
                                </div>
                                ...
                                {(flag.doc || flag.docs) && (
                                  <div className="flex gap-2 mt-2">
                                    ...
                                    >
                                      Go to Deed <ExternalLink className="w-2.5 h-2.5" />
                                    </button>
                                  </div>
                                )}
                              </div>
        ...
        {activeTab === "valuation" && (
          <div className="h-full w-full overflow-y-auto animate-in fade-in duration-700 p-8">
            <div className="max-w-6xl mx-auto space-y-12">
              ...
              {/* New Comparative Table */}
              <ValueComparisonAudit results={results} />

              <div className="mt-16 pt-12 border-t">
                <div className="mb-8">
                  <h2 className="text-3xl font-black text-slate-900">Historical Market Trends</h2>
                  <p className="text-slate-500">EC-driven property valuation analysis across all recorded years</p>
                </div>
                <ECHistoricalValues data={valuationData || []} isLoading={loadingValuation} />
              </div>
            </div>
          </div>
        )}
